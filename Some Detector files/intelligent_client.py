...
class IntelligentPingPongDetector:
    ...
    def _evaluate_clusters(self, state: Dict, clusters: List[List[Dict]]):
        ...
        for cluster in clusters:
            ...
            # RSRP-aware optimization: find radio-quality optimal position
    ...
    def _path_loss_estimate(self, anchor_pos: Dict, ue_pos: Dict) -> float:
        """
        Estimate RSRP (dBm) from anchor to UE using ITU path loss model.
        Formula: PL(d) = 20*log10(f) + 20*log10(d) + L0
        Returns RSRP in dBm (higher is better; >-100 dBm is acceptable).
        """
        distance = self._dist(anchor_pos, ue_pos)
        if distance < 1:
            distance = 1.0
        
        # Frequency 2.6 GHz: 20*log10(2600) ≈ 68.3
        path_loss_db = 68.3 + 20 * math.log10(distance) + 32.5
        
        # TX power 50 dBm minus path loss
    ...
    def _optimize_anchor_position(self, centroid: Tuple[float, float], 
        ...
        """
        Grid search to find anchor position that maximizes minimum RSRP.
        Replaces simple distance-based centroid with radio-quality optimization.
        """
        cx, cy = centroid
        grid_size = 5  # pixels per search step
        search_radius = self.r_anchor  # Search within coverage radius
        
        best_pos = (cx, cy)
        best_min_rsrp = -200  # Worst possible
        
        # Grid search in square around centroid
        for dx in range(-int(search_radius), int(search_radius) + 1, grid_size):
            for dy in range(-int(search_radius), int(search_radius) + 1, grid_size):
                ...
                # Compute minimum RSRP to all UEs in cluster
                min_rsrp = float('inf')
                for ue in cluster:
                    rsrp = self._path_loss_estimate(test_pos, ue)
                    min_rsrp = min(min_rsrp, rsrp)
                
                # Keep position with highest minimum RSRP
                if min_rsrp > best_min_rsrp:
                    best_min_rsrp = min_rsrp
                    best_pos = (test_x, test_y)
        
        self._log(f"RSRP-optimized: ({best_pos[0]:.0f},{best_pos[1]:.0f}) "
