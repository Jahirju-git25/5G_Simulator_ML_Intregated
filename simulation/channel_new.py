...
class ChannelModel:
    ...
    SPEED_OF_LIGHT = 3e8
    LOG_DIST_D0 = 1.0
    LOG_DIST_SIGMA_DB = 8.0

    def __init__(self, scenario='UMa', pathloss_model='3GPP', log_dist_n=3.5,
                 log_dist_shadow='lognormal', fading_model='Rayleigh'):
        self.scenario = scenario
        self.pathloss_model = pathloss_model
        self.log_dist_n = float(log_dist_n)
        self.log_dist_shadow = log_dist_shadow
    ...
    def calculate_sinr(self, rsrp_dbm, interference_list_dbm, noise_figure_db=7):
        signal_mw = 10**(rsrp_dbm/10)
        noise_mw = 10**(self.thermal_noise_dbm/10)*10**(noise_figure_db/10)
    ...
    def calculate_rsrq(self, rsrp_dbm, rssi_dbm):
        return 10*math.log10(66)+rsrp_dbm-rssi_dbm
    ...
    def _uma_pathloss(self, d_2d, h_bs=25, h_ut=1.5):
        ...
        if is_los:
            if d_2d <= d_bp:
                ...
                pl = 28.0+40*math.log10(d_3d)+20*math.log10(fc)-9*math.log10(d_bp**2+(h_bs-h_ut)**2)
            sigma = 4.0
        else:
            pl_nlos = 13.54+39.08*math.log10(d_3d)+20*math.log10(fc)-0.6*(h_ut-1.5)
            pl = max(pl_nlos, 28.0+22*math.log10(d_3d)+20*math.log10(fc))
            sigma = 6.0
    ...
    def _umi_pathloss(self, d_2d, h_bs=10, h_ut=1.5):
        ...
        c = self.SPEED_OF_LIGHT
        h_e = 1.0
        d_bp = 4*(h_bs-h_e)*(h_ut-h_e)*self.FREQ_HZ/c
        d_3d = math.sqrt(d_2d**2+(h_bs-h_ut)**2)
        ...
        if is_los:
            if d_2d <= d_bp:
                ...
                pl = 32.4+40*math.log10(d_3d)+20*math.log10(fc)-9.5*math.log10(d_bp**2+(h_bs-h_ut)**2)
            sigma = 4.0
        else:
            pl_nlos = 35.3*math.log10(d_3d)+22.4+21.3*math.log10(fc)-0.3*(h_ut-1.5)
            pl = max(pl_nlos, 32.4+21*math.log10(d_3d)+20*math.log10(fc))
            sigma = 7.82
        pl += random.gauss(0, sigma)+self._apply_fading()
        return pl, is_los
    ...
    def _rma_pathloss(self, d_2d, h_bs=35, h_ut=1.5, w=20, h=5):
        ...
        if is_los:
            if d_2d <= d_bp:
                pl = (20*math.log10(40*math.pi*d_3d*fc/3)+min(0.03*h**1.72, 10)*math.log10(d_3d) -
                      min(0.044*h**1.72, 14.77)+0.002*math.log10(h)*d_3d)
            else:
                pl = (20*math.log10(40*math.pi*d_bp*fc/3)+min(0.03*h**1.72, 10)*math.log10(d_bp) -
                      min(0.044*h**1.72, 14.77)+0.002*math.log10(h)*d_bp+40*math.log10(d_3d/d_bp))
            sigma = 4.0
        else:
    ...
    def _cqi_throughput(self, sinr_db, bandwidth_hz):
        cqi_table = [(-10, 1e6), (-6, 2e6), (-3, 5e6), (0, 10e6), (3, 20e6), (6, 40e6), (9, 70e6),
                     (12, 110e6), (15, 160e6), (18, 220e6), (21, 280e6), (24, 350e6), (27, 420e6),
                     (30, 480e6), (33, 550e6)]
        throughput = 0
