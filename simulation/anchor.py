    ...
    def assign_dc_with_external_anchor(
        ...
        with self.sim.lock:
            ...
            with self._lock:
                ...
                for gnb in self.sim.gnbs.values():
                    if gnb.id != anchor_gnb_id:
                        try:
                            gnb.connected_ues.remove(ue_id)
                        except ValueError:
                            pass
    ...
    def _generate_ho_chart(self, ue_id, serving_gnb_id, target_gnb_id,
        ...
        try:
            ...
            with _MATPLOTLIB_LOCK:
                ...
                import gc
                # (remove the close-all block entirely — per-figure plt.close(fig)
#                 #  in the finally already handles cleanup correctly)
