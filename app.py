...
_detector_status_lock = threading.Lock()
...
def set_ue_mobility():
    ...
    with simulator.lock:
        ...
        from simulation.mobility_new import MobilityModel
...
def detector_status_handler():
    """
    Handle detector status queries and updates from external ML detector.
    GET: Returns current detector status
    POST: Updates detector status with provided fields
    """
    ...
    with _detector_status_lock:
        if 'evaluation_steps' in data:
            detector_status['evaluation_steps'] = data.get('evaluation_steps', 0)
        if 'active_anchors' in data:
            detector_status['active_anchors'] = data.get('active_anchors', [])
        if 'cost_benefit_rejections' in data:
            detector_status['cost_benefit_rejections'] = data.get('cost_benefit_rejections', 0)
        if 'false_positives' in data:
            detector_status['false_positives'] = data.get('false_positives', 0)
        if 'ue_count' in data:
            detector_status['ue_count'] = data.get('ue_count', 0)
        if 'errors' in data:
            detector_status['errors'] = data.get('errors', 0)
        if 'dc_assignments' in data:
            detector_status['dc_assignments'] = data.get('dc_assignments', 0)
        if 'dc_smart_skipped' in data:
            detector_status['dc_smart_skipped'] = data.get('dc_smart_skipped', 0)
        if 'hos_avoided' in data:
            detector_status['hos_avoided'] = data.get('hos_avoided', 0)
        if 'figures_saved' in data:
            detector_status['figures_saved'] = data.get('figures_saved', 0)
        
        return jsonify({'success': True, 'message': 'Detector status updated', 'status': dict(detector_status)})
...
def deploy_anchor():
    ...
    Creates a new AnchorGNB at the specified canvas position.
    """
    d = request.json or {}
    x           = float(d.get('x', 400))
    y           = float(d.get('y', 300))
    ...
    simulator._log_event(
        f"[AnchorGNB] {gid} deployed @ ({x:.0f},{y:.0f}) "
        f"TX={tx_power}dBm sectors={num_sectors} "
        f"triggered by {triggered_by} ({ho_count} HOs)")
    ...
    return jsonify({
        ...
        'gnb_id': gid,
        'anchor_gnb_id': gid,
        'message': f'AnchorGNB {gid} deployed at ({x:.0f},{y:.0f})'
    })
...
def assign_dc():
    ...
    Enables anchor as secondary gNB for the UE.
    
    Parameters:
      ue_id: Target UE
      anchor_gnb_id: Anchor gNB to assign as secondary
      preferred_senb_id (optional): Preferred serving eNB
    """
    d = request.json or {}
    ue_id = d.get('ue_id')
    anchor_gnb_id = d.get('anchor_gnb_id')
    
    if not ue_id or not anchor_gnb_id:
        return jsonify({
            'success': False,
            'error': 'ue_id and anchor_gnb_id are required'
        }), 400
    
    with simulator.lock:
        ue = simulator.ues.get(ue_id)
        anchor = simulator.gnbs.get(anchor_gnb_id)
        
        if not ue:
            return jsonify({
                'success': False,
                'error': f'UE {ue_id} not found'
            }), 404
        
        if not anchor:
            return jsonify({
                'success': False,
                'error': f'Anchor {anchor_gnb_id} not found'
            }), 404
        
        # Enable dual connectivity
        ue.dc_enabled = True
        ue.anchor_gnb_id = anchor_gnb_id
        ue.secondary_gnb_id = anchor_gnb_id
        
        # Add UE to anchor's connected UEs
        if ue_id not in anchor.connected_ues:
            anchor.connected_ues.append(ue_id)
        
        simulator._log_event(
            f"[DC] {ue_id} assigned to anchor {anchor_gnb_id} (dual connectivity enabled)")
    
    return jsonify({
        'success': True,
        'message': f'UE {ue_id} assigned to anchor {anchor_gnb_id}',
        'ue_id': ue_id,
...
@app.route('/api/anchor/assign', methods=['POST'])
def anchor_assign():
    d = request.json or {}
    ue_id = d.get('ue_id')
    anchor_gnb_id = d.get('anchor_gnb_id') or d.get('anchor_id')
    preferred_senb_id = d.get('preferred_senb_id') or d.get('senb_id')
    if not ue_id or not anchor_gnb_id:
        return jsonify({
            'success': False,
            'error': 'ue_id and anchor_gnb_id are required'
        }), 400
    ok, msg = simulator.anchor_manager.assign_dc_with_external_anchor(
        ue_id, anchor_gnb_id, preferred_senb_id)
    return jsonify({'success': ok, 'message': msg})
